...
export async function POST(req:NextRequest) {
    try {
        ...
        { success: false, data: null, message: "Cannot have an empty comment!" },
        { status: 400 }   //bad req
      ); 

        }
        const existingDeal=await collection.findOne({userid:session?.user.id,link:link});
      ...
      return NextResponse.json(
        { success: false, data: null, message: "Addition of comment unsuccessful" },
        { status: 500 }
      );
    }
