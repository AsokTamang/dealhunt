...
export async function POST(req: NextRequest) {
  ...
  try {
    const existingDeal = await collection.findOne({
      ...
      title,
      link,
    }); //first we need to check if the link is already saved or not.
    if (!existingDeal) {
      //if not then only we save the link
      const saveDeal = await collection.insertOne({
        userid: session?.user.id,
        title,
        link,
      }); //here we are saving the userid with title as well as link and ofcourse the id of the user also.
    ...
    if (error instanceof Error) {
      ...
      return NextResponse.json({
        success: false,
        data: null,
        message: "failed saving the deal",
      });
    }
  }
}
